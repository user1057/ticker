...
public class HuobiMarketService implements MarketClient {
  ...
  @Override
  public HuobiWebSocketConnection subMarketBBOv2(SubMarketBBORequest request, ResponseCallback<MarketBBOEvent> callback) {
    ...
    return HuobiWebSocketConnection.createMarketConnection(options, commandList, new MarketBBOEventParser(), callback, false);
  ...
  public HuobiWebSocketConnection subMbpRefreshUpdateV2(SubMbpRefreshUpdateRequest request, ResponseCallback<MbpRefreshUpdateEvent> callback) {
    ...
    List<String> commandList = new ArrayList<>(symbolList.size());
    long finalClientId = System.nanoTime();
    symbolList.forEach(symbol -> {
      ...
      JSONObject command = new JSONObject();
      command.put("sub", topic);
      command.put("id", finalClientId);
      commandList.add(command.toJSONString());
    });

    return HuobiWebSocketConnection.createMarketConnection(options, commandList, new MbpRefreshUpdateEventParser(), callback, false);
  }

  public void unsubMbpRefreshUpdate(SubMbpRefreshUpdateRequest request, ResponseCallback<MbpRefreshUpdateEvent> callback, Long clientId) {
